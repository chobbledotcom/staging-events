---
title: "Prostate Cancer UK"
subtitle: "March for Men"
description: "Full production and logistics support for Prostate Cancer UK's March for Men events across the UK."
header_image: images/prostate-cancer-uk-march-for-men-stage.jpg
blocks:
  - type: image-background
    parallax: true
    image: images/prostate-cancer-uk-march-for-men-stage.jpg
    image_alt: Prostate Cancer UK March for Men stage
    content: |
      # Prostate Cancer UK

      ## March for Men

      Full production and logistics for the UK-wide fundraising walks.

  - type: split-image
    title: About Prostate Cancer UK
    reverse: true
    reveal_content: right
    reveal_figure: scale
    content: |
      Prostate Cancer UK is the UK's leading men's health charity, funding world-class research, providing information and support, and campaigning for a better deal for men with prostate cancer.
    figure_src: images/prostate-cancer-uk-logo.jpg
    figure_alt: Supporting Prostate Cancer UK logo

  - type: split-image
    title: Delivering March for Men
    reveal_content: left
    content: |
      We handle the full production and logistics for Prostate Cancer UK's March for Men, which is one of the charity's main fundraising events.

      That covers all of it from our end: staging, sound, start and finish gantries, and the logistics to keep things running across multiple event locations.
    figure_src: images/prostate-cancer-uk-march-for-men-start-gantry.jpg
    figure_alt: Prostate Cancer UK March for Men start gantry

  - type: split-image
    title: Stage, Sound and LED
    reverse: true
    reveal_content: right
    content: |
      The full production package includes a main stage, PA, LED screen and the start and finish gantries, all branded throughout for Prostate Cancer UK. Our crew rigs and runs the equipment at each location, so the day stays on schedule without the charity team having to manage that side.
    figure_src: images/prostate-cancer-uk-march-for-men-led-screen.jpg
    figure_alt: LED screen at Prostate Cancer UK March for Men

  - type: gallery
    items:
      - image: /images/prostate-cancer-uk-branded-stage-pa.jpg
      - image: /images/march-for-men-crowd-branded-gazebos.jpg
      - image: /images/march-for-men-route-signpost.jpg

  - type: icon-links
    intro_content: |
      ## What We Provide
    items:
      - icon: "mdi:tools"
        text: Full production support
      - icon: "mdi:clipboard-list"
        text: Logistics coordination
      - icon: "mdi:speaker"
        text: Stage and PA hire
      - icon: "mdi:gate"
        text: Start/finish gantries
      - icon: "mdi:account-group"
        text: Event staffing
      - icon: "mdi:bottle-tonic"
        text: Water supply and distribution

  - type: cta
    title: Plan Your Nationwide Event
    description: >-
      Get in touch and we'll go through production and logistics for a mass participation walk, however many venues are involved.
    button:
      text: Contact Us
      href: /contact/
      variant: secondary
      size: lg
---
